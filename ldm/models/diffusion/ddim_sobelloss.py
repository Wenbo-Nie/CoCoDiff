...
class DDIMSamplers(object):
    ...
    def _calculate_sobel_loss(self, generated_image_tensor, content_image_tensor):
        ...
        if generated_image_tensor.shape[1] == 3:
            generated_image_gray = K.rgb_to_grayscale(generated_image_tensor)
            content_image_gray = K.rgb_to_grayscale(content_image_tensor)
        else:
            generated_image_gray = generated_image_tensor
            content_image_gray = content_image_tensor

        edges_gen = K.sobel(generated_image_gray)
        edges_content = K.sobel(content_image_gray)
    ...
    def apply_sobel_guidance(self, pred_x0_unbiased, current_noisy_img, time_step_idx, 
        ...
        print(f"sobel_loss:",sobel_loss_value.mean())
    ...
    def ddim_sampling(self, cond, shape, negative_conditioning=None,
        ...
        for i, step in enumerate(iterator):
            ...
            if style_guidance > 0 and style_img is not None:
                ...
                pred_x0_guided = self.apply_sobel_guidance(
